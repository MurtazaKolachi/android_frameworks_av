...
std::string getVendorXmlPath(const std::string &path) {
    ...
    if (!vendorPath.empty()) {
        if (fileExists(vendorPath + std::string(".xml"))) {
            ...
            targetVersion = android::base::GetProperty("vendor.media.target.version", "0");
#else
            targetVersion = android::base::GetProperty("vendor.sys.media.target.version", "0");
#endif
            int version = atoi(targetVersion.c_str());
            if (version > 0) {
