            ...
            if (xmlFile == nullptr) {
                ...
                if (!strncmp(value, "/vendor/etc", strlen("/vendor/etc"))) {
                    ...
                    if (!strcmp(platform, "msm8953")) {
                        std::string targetVersion = android::base::GetProperty("vendor.media.target.version", "0");
                        if (atoi(targetVersion.c_str()) == 1 &&
                        ...
                        std::string targetVersion = android::base::GetProperty("vendor.sys.media.target.version", "0");
                        if (atoi(targetVersion.c_str()) == 3 &&
                                checkXmlFile("/vendor/etc/media_profiles_khaje.xml")) {
                            strlcpy(value, "/vendor/etc/media_profiles_khaje.xml",
                                    PROPERTY_VALUE_MAX);
                        } else if (atoi(targetVersion.c_str()) == 2 &&
